        ...
        <div class="px-3 py-2 border-bottom mb-0" style="background-color: #E4E4E4;">
            <div class="container">
                <div class="d-flex flex-wrap align-items-center justify-content-center justify-content-lg-start">
                    <form action="results.php" method="get" class="d-flex me-lg-auto" role="search">
                        <input type="search" name="search" class="form-control me-2" placeholder="Search products..." aria-label="Search">
                        <button class="btn btn-outline-dark" type="submit">Search</button>
                    </form>
                    <ul class="nav col-12 col-lg-auto my-2 justify-content-center my-md-0 text-small float-end">
                        <li><a href="cart.php" class="nav-link text-dark position-relative">
                            <i class="fa fa-shopping-cart" aria-hidden="true"></i>
                            <?php
                            // Show cart item count if user is logged in
                            if (isset($_SESSION['username']) && isset($con)) {
                                ...
                                if ($stmt = mysqli_prepare($con, $count_sql)) {
                                    ...
                                    if (mysqli_stmt_fetch($stmt) && $cart_count > 0) {
            ...
            <div class="container">
                <div class="d-flex justify-content-center py-3">
                    <ul class="nav nav-pills">
                        <li class="px-5 py-0 nav-item">
                            <a href="home.php" class="nav-link nav-elem text-light fs-5 text-uppercase fw-light me-5">Smart Fashion</a>
                        </li>
                        <li class="nav-item">
                            <a href="home.php" class="nav-link nav-elem text-white mt-1 mx-5" aria-current="page">Home</a>
                        </li>
                                ...
                                if (isset($con)) {
                                    ...
                                    if ($result && mysqli_num_rows($result) > 0) {
                                        while($row = mysqli_fetch_assoc($result)) {
                                            $collection = htmlspecialchars($row['collection'], ENT_QUOTES, 'UTF-8');
                                            echo '<li><a class="dropdown-item text-white" href="collection.php?collection=' . urlencode($row['collection']) . '">' . $collection . '</a></li>';
                                        }
                                    } else {
                                        echo '<li><a class="dropdown-item text-white" href="#">No collections</a></li>';
                                    }
                        ...
                        <li class="nav-item">
                            <a href="lookbook.php" class="nav-link nav-elem text-white mt-1 mx-3">Lookbook</a>
                        </li>
                        <li class="nav-item">
                            <a href="customer_care.php" class="nav-link nav-elem text-white mt-1 mx-3">Customer Care</a>
                        </li>
                    </ul>
                </div>
            </div>
