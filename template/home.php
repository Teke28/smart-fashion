...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['newsletter_submit'])) {
    ...
    if (filter_var($newsletter_email, FILTER_VALIDATE_EMAIL)) {
        ...
        $line = date('c') . ' ' . $newsletter_email . PHP_EOL;
        if (@file_put_contents($file, $line, FILE_APPEND | LOCK_EX) !== false) {
            echo "<script>alert('Thank you — you have been subscribed.');</script>";
        } else {
            echo "<script>alert('Subscription failed. Please try again later.');</script>";
        }
    } else {
...
// Fix: Use consistent mysqli approach
$sql = "SELECT * FROM products LIMIT 3";
$all_product = mysqli_query($con, $sql);
if (!$all_product) {
    error_log('Product query error: ' . mysqli_error($con));
    $all_product = false;
}
?>
<!DOCTYPE html>
...
<body>
    ...
    <div class="container cont_new">
        ...
        <div class="cont-card">
            ...
            ?>
            <a href="product.php?pId=<?php echo (int)$row['pId']; ?>" class="card border-0" style="width: 18rem;">
                <img src="<?php echo htmlspecialchars($row['pImg'], ENT_QUOTES, 'UTF-8'); ?>" class="card-img-top" alt="<?php echo htmlspecialchars($row['pName'], ENT_QUOTES, 'UTF-8'); ?>">
            </a>
            <?php 
                }
            } else { 
    ...
    <div class="container cont-on_list text-center" style="width: 45vw;">
        ...
        <form method="post" action="">
            <div class="mb-3">
                <input type="email" name="newsletter_email" class="form-control" id="exampleInputEmail1" aria-describedby="emailHelp" placeholder="enter your email address" required />
            </div>
            <button type="submit" name="newsletter_submit" class="w-50 btn btn-lg text-light" style="background-color: #A3C7BD;">Submit</button>
        </form>
    </div>
